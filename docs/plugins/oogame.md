...
新增 Java 21 兼容线是未来目标，**不是当前开发版或已发布版本的支持声明**。MC/Paper/Folia 跨版本识别、映射与适配统一由 OOCore 实现，OOGame 不另建兼容层；本项目只保证自身 Java 构建基线、必要依赖，并在 OOCore 提供的能力上验证游戏业务。计划由 OOCore 先覆盖 Paper 1.20.5/1.20.6 与 1.21–1.21.11 的适配边界，再核更早 1.20.x；同时保留 Java 25 / 26.x 方向。当前 OOGame 仍以 Java 25 编译，依赖及真实玩家流程尚未完成 Java 21 验收。仅升级 OOCore 不能使 Java 25 字节码直接在 Java 21 加载，也不保证未来所有破坏性变化都无需消费者调整。
