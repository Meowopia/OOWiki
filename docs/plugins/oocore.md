...
- **开发快照不是正式版本**：当前开发树为 **1.8.0-SNAPSHOT**，尚不具备正式发布条件；没有新的 Release、安装包或正式 SDK 下载。新增平台能力仍需完成契约、平台与实服验证，离线测试和隔离构建不等于正式支持。
- **通道与平台能力仍受限**：1.8 通道 I/O 生产入口保持关闭，仍待宿主部署批准与加载来源验证；新增 Folia 能力尚未验收，不能作为可用功能部署。
- **优先继续性能定位**：使用可比较的负载检查内存分配、存活对象、GC 暂停与 MSPT 尾部尖峰，再决定后续优化；不先承诺节省比例或加速幅度。
- **改进存储扩展能力**：有界异步存储已进入 **Code-prepared / 测试中**，尚未正式提供。已完成定向磁盘与生命周期测试，覆盖排队上限、内存预算、退出撤销，以及写入结果与持久性保障的区分；事务型迁移与恢复工具不因此视为已交付。定向测量仅表明等待磁盘的工作移出了调用线程，不证明 TPS、MSPT 或总内存改善。完整 Paper 异步负载、其他平台及真实业务联动仍待验收。
