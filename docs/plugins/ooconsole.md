...
!!! danger "已归档 / Archived"
    **OOConsole 已取消后续开发并归档，现已停止开发和维护。** 本页保留历史正式版 **0.1.6** 的安装、配置与功能事实，供已有用户查阅；不再推荐新部署，也不承诺完成下述未交付功能。归档不表示历史版本已实现的能力失效，也不表示 OOEngine 已自动接管或合并 OOConsole。

OOConsole 是喵托邦（Meowopia）OO 系列曾经提供的管理与可视化编辑插件，属于基础（Core）。历史正式版 **0.1.6** 提供管理基础能力：状态命令、插件贡献接口，以及可选的本机 HTTP 登录、会话与只读接口。**它从未成为完整的网页管理面板。**
...
## 已取消的历史规划 / Cancelled roadmap

- 曾计划沿用 OOEngine 的正式编辑与窗口能力，完善结构化编辑、校验、预览、版本检查及保存发布流程。
- 曾计划通过已适配插件的受控贡献逐步提供管理工作区。
- 曾计划开放带版本、校验、中英说明及升级回滚的配置。

这些内容均未交付，现随项目归档取消，不再构成路线图或版本承诺。

## 安装

> **归档提示：** 以下步骤仅供已经持有历史正式版的用户维护或恢复现有安装，不建议新部署。请保留已验证 JAR 和完整备份。

1. 先准备能够正常运行的 Paper 服务端，以及 **OOCore、OOEngine** 两个前置插件。缺少任意一个，历史版 OOConsole 都无法启动。
...
Java 21 支持曾是兼容目标，但没有随 0.1.6 交付，并已随项目归档取消。0.1.6 仍只能按原有 Java 25 服务端环境理解。

曾计划验证 Paper 1.20.5、1.20.6 及 1.21–1.21.11 的相关版本，再评估更早的 1.20.x；这些组合没有完成验收，也不再安排交付。
...
OOConsole 在归档前未完成旧 OOEngine Web Editor 的功能等价迁移。旧编辑器是否保留及如何演进由 OOEngine 自身版本决定；安装历史版 OOConsole 不能作为删除旧编辑器或迁移其数据的依据。
