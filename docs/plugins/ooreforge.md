...
**可视化管理不可用。** OOConsole 已归档，且从未交付可供服主启用的锻造工作区；当前也没有已验收的 OOEngine 客户端窗口。不要寻找或自行搭建另一个独立 Web 后台。游戏内工作台布局编辑与网页配置管理是不同功能。
...
OOReforge 新增 Java 21 兼容目标，同时保留 Java 25 / 26.x 开发方向。**这是未来兼容目标，不是当前版本已支持声明；项目仍未正式发布。**

| 目标服务端 | 目标运行 Java | 当前状态 |
| --- | --- | --- |
| Paper 1.20.5、1.20.6 | Java 21 | 优先兼容目标，尚未验收 |
| Paper 1.21–1.21.11 | Java 21 | 按 API 差异分批验证，尚未验收 |
| 更早的 Paper 1.20.x | Java 21 | 后续核查范围，不承诺已兼容 |
| Paper 26.1+ | Java 25 | 保留现有开发方向，不代表已有稳定交付 |

[Paper 官方 Java 推荐表](https://docs.papermc.io/paper/getting-started/)将 1.20–1.21.11 对应 Java 21、26.1+ 对应 Java 25。不能让 26.x 服务端改用 Java 21，也不能仅修改插件配置就获得跨版本兼容。

兼容工作须先满足 OOCore 必需前置及所选可选插件的兼容条件，再验证装备、材料、支付与数据迁移。Folia 需要独立验收；本计划不增加 Spigot 或所有历史 Minecraft 版本的支持承诺。不建议将新版本服务器的装备数据直接降级到旧版本。

跨 Minecraft / Paper / Folia 版本的识别、映射与平台适配统一由 **OOCore** 提供；OOReforge 使用其稳定接口，不另建版本兼容层。OOReforge 自身及必要依赖仍需满足 Java 21 的加载要求，并在 OOCore 已支持的平台上验证锻造业务。这不表示仅升级 OOCore 就一定能够解决所有未来变更；尚未验收的兼容范围仍为计划，不是已发布支持。
